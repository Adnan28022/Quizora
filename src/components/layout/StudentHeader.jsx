...
import { Bell, Search, User, Menu, PanelLeftClose, PanelLeftOpen, LogOut, Settings, Award } from 'lucide-react';
                ...
                <div className="relative" ref={profileRef}>
                    <button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-3 p-1 pr-3 hover:bg-slate-50 rounded-2xl transition-all group">
                        ...
                        <div className="text-left hidden xl:block leading-tight">
                            <p className="text-xs font-black text-slate-900 uppercase">Alex Johnson</p>
                            <p className="text-[9px] font-bold text-indigo-500 uppercase tracking-wider italic">Rank #42</p>
                        </div>
                    </button>
                    ...
                    {profileOpen && (
                        <div className="absolute right-0 mt-3 w-60 bg-white border border-slate-100 rounded-[1.5rem] shadow-2xl p-2 animate-in fade-in zoom-in-95 duration-200">
                            <div className="p-4 border-b border-slate-50 mb-1">
                                <p className="text-[10px] font-black text-slate-400 uppercase leading-none mb-1">Account ID: 9021</p>
                                <p className="text-xs font-bold text-slate-800 truncate">alex@quizora.com</p>
                            </div>
                            <div className="space-y-0.5 text-slate-600">
                                <button className="w-full flex items-center gap-3 px-4 py-2.5 text-[11px] font-bold hover:bg-indigo-50 hover:text-indigo-600 rounded-xl transition-all"><User size={16} /> Profile View</button>
                                <button className="w-full flex items-center gap-3 px-4 py-2.5 text-[11px] font-bold hover:bg-indigo-50 hover:text-indigo-600 rounded-xl transition-all"><Award size={16} /> My Certificates</button>
                                <button className="w-full flex items-center gap-3 px-4 py-2.5 text-[11px] font-bold hover:bg-indigo-50 hover:text-indigo-600 rounded-xl transition-all"><Settings size={16} /> Settings</button>
                            </div>
